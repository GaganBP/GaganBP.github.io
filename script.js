...
const typewriterElement=document.querySelector(".typewriter-text");if(typewriterElement){const shouldRunTypewriter=optimizeForMobile();if(shouldRunTypewriter!==!1){if(isMobileDevice()){requestAnimationFrame(()=>{setTimeout(initTypewriter,1500)})}else{requestAnimationFrame(()=>{setTimeout(initTypewriter,500)})}}}}catch(error){console.warn('Portfolio initialization error:',error)}}
...
function optimizeForMobile(){if(!isMobileDevice())return;const floatingElements=document.querySelectorAll('.floating-element');floatingElements.forEach(el=>{if(el){el.style.animation='none';el.style.transform='translateZ(0)'}});setTimeout(()=>{const cards=document.querySelectorAll('.project-card, .service-card');cards.forEach(card=>{if(card){card.style.willChange='auto'}})},2000);return!0}
...
const phrases=["Freelance Data Analyst","Power BI Developer","Excel Dashboard Specialist","Data Cleaning Expert","Python + AI Analyst",];let currentPhrase=0;let currentChar=0;let isDeleting=!1;function initTypewriter(){const el=document.querySelector(".typewriter-text");if(!el){return}
const isMobileForTypewriter=isMobileDevice();const typeSpeed=isMobileForTypewriter?80:60;const eraseSpeed=isMobileForTypewriter?40:30;const delayBetween=isMobileForTypewriter?1500:1200;let typeTimeout;function type(){const phrase=phrases[currentPhrase];if(!isDeleting){el.textContent=phrase.slice(0,currentChar+1);currentChar++;if(currentChar===phrase.length){isDeleting=!0;typeTimeout=setTimeout(type,delayBetween)}else{typeTimeout=setTimeout(type,typeSpeed)}}else{el.textContent=phrase.slice(0,currentChar);currentChar--;if(currentChar===0){isDeleting=!1;currentPhrase=(currentPhrase+1)%phrases.length;typeTimeout=setTimeout(type,typeSpeed)}else{typeTimeout=setTimeout(type,eraseSpeed)}}}
if(isMobileForTypewriter){el.style.willChange='auto';el.style.transform='translateZ(0)'}
type();return()=>{if(typeTimeout){clearTimeout(typeTimeout)}}}
