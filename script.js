...
function optimizeForMobile(){if(!isMobileDevice())return!0;const floatingElements=document.querySelectorAll('.floating-element');floatingElements.forEach(el=>{if(el){el.style.animation='none';el.style.transform='translateZ(0)'}});setTimeout(()=>{const cards=document.querySelectorAll('.project-card, .service-card');cards.forEach(card=>{if(card){card.style.willChange='auto'}})},2000);return!0}
